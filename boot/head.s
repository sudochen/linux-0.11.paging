...
#
# swapper_pg_dir是页目录的地址，页目录地址在0x00000000处
# 根据setup模块我们安装了两个段描述符，分别是代码段和数据段，共8MB的空间
# 0x10的二进制为10000, 
# 我们可以看出DPL为0，T为0表示此描述符在全局描述符中，
# 10表示第二个描述符，权限为0最高权限
# 综上，设置数据段为0x10，全局描述第2个段
# 也就是设置数据段，数据段的基地址为0，长度为8MB
#
swapper_pg_dir:
startup_32:
	cld
	movl $0x10,%eax		# 0x10 = 0b00010000 表示索引为2，权限为最高权限（00）的全局段描述符
	mov %ax,%ds
	mov %ax,%es
	mov %ax,%fs
	mov %ax,%gs
...
# 使用eax作为低32位和edx高32位构造中断门描述符
# 在这里扩展一下
# X86下的描述符分为段描述符和门描述符，都是8个字节
# 段描述符有代码段描述符，数据段描述符，系统段描述符（LDT，TSS）
# 门描述符有陷阱门描述符，中断门描述符，调用门描述符，任务门描述符
# 
#
setup_idt:
	lea ignore_int,%edx		# 获取ignore_int的有效地址，edx为中断门的高32位
	movl $0x00080000,%eax	# eax寄存器为中断门的低32位
	movw %dx,%ax			# selector = 0x0008 = cs
	movw $0x8E00,%dx		# interrupt gate - dpl=0, present
#
# 上述的代码构造中断门描述符， eax维低32位，edx为高32位，CS为8表示二进制为00001000表示段选择子为1（代码段）
# +----------+---+-----------+---+---+-----------+--------+--------------+-----------+----------+
...
# 分页是怎么进行的？
# 分页机制将线性地址分成三个部分进行查表，
# 1，通过高10位在页目录中查找获取一个页表，
# 2，通过中10位在页表中查找获取一个页，低12位表示偏移，
# 在寻址时，根据高10位找到页表，根据中10位在页面中找到对应的物理页起始地址，在加上低12位的偏移形成物理地址
# 
# 页目录和页表以4KB对齐，对于页目录项和页表项的值，低12位有其特殊的意义，具体可参考intel的数据手册
# 
# 这20K中前4K为页目录，页目录中一共有1024个页目录项，共占用4K空间
# 一个页目录项有1024个页表，一个页表项代表一个页，一个页是4K大小，因此一个页目录项可维护4MB内存
# 1024个页目录项可维护1024*4MB = 4GB的内存
#
# 由于当前系统中只用16MB的内存，因此我们只需要4个页表项即可
# 
#
#
...
# 为什么是3072，一个页目录项占用4个字节，那么3072正好对应的是第（3072/4 = 768）个页目录项
# 那第768个目录项维护内存的起始地址是 768*4M = 3072M = 3G = 0xC0000000地址处
# 因此3072表示的是地址0xC0000000在页目录的偏移
...
#  0x00004 007
# 一共5个页表目录项有效
# 又上我们可以看到0xC0000000和0x00000000存放的是同一个页表，这一点非常重要
#
#
	...
	std
# 
# stosl指令将EAX中的值保存到ES:EDI指向的地址中 ES:EDI = EAX
# 如果在此之前使用std将eflags的方向位置位则每次EDI自减4，否则自加4
# 执行完毕后建立了16MB的页表page0,page1,page2,page3
#
...
# 
# 上面的代码执行完成后内存如下
# 地址0x00000000 PDT (Page Directory Table) swapper_pg_dir
#  0x00001 007
# 地址0x00000C00
#  0x00001 007
#  0x00002 007
#  0x00003 007
#  0x00004 007
# 地址0x00001000 pg0 PT (Page Table)
#  0x00000 007
#  0x00001 007
# 地址0x00002000 pg1
#  0x00400 007
...
#
# 一个例子：
# 如果要访问地址0x00FFF111地址，我们根据规则将其分解，
# 我们知道系统需先经过段映射由于段映射后线性地址和逻辑地址一样
# 高10位为0b0000000011即3，第三项页面目录项其对应的页面项为pg3
# 中10位为0b1111111111即1023，每一个页面占4个地址即1023*4 = 4092，也就是pg3的最后一项，对应为0xfff
# 低12位为偏移地址0x111
# 合并起来后地址为0x00FFF111，
# 我们可以看出页管理后系统还是平坦映射
#
