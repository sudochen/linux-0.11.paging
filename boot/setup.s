...
# 之所以能运行是因为bootsect的最后一句代码如下
# ljmp $SETUPSEG, $0
# 这句代码已经改变了CS的内容，而_start所处的正是0地址
# 
_start:
...
# 问：为什么在这要把system模块移动到0地址处，为什么从一开始不移动
# 答：因为setup模块的开始需要用到BIOS系统调用获取参数，因此只能在获取参数后移动system模块
# 此后BIOS调用就不能用了
#
	mov	$0x0000, %ax
...
# 将SETUPSEG作为段地址
# lidt lgdt加载中断描述符表和全局描述符表
# 由于setup模块也是从0地址链接的，此处直接使用标号
#
#
end_move:
...
# +--------------------+---------------+------------------+
# $8的二进制为00001000,根据以上可以看出是索引为1，最高权限（00）的全局描述符（0）
# 每个描述符占8个字节
#
